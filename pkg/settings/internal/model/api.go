...
type Api struct {
	...
	Status      types.ApiStatusType `xorm:"TINYINT(2) default 1 comment('上线状态：1 下线，2 上线')"`

	// 定义API请求
	ReqPath          string                    `xorm:"VARCHAR(255) comment('请求Path')"`
	ReqHTTPMethod    types.HTTPMethod          `xorm:"VARCHAR(50) not null comment('请求方法：GET，POST，PATCH，PUT，DELETE，HEAD，OPTIONS，ANY')"`
	InputRequestMode types.ApiInputRequestMode `xorm:"VARCHAR(100) not null comment('入参请求模式：MAPPING 入参映射（过滤未知参数），MAPPING_PASS_THROUGH 入参映射（透传未知参数），PASS_THROUGH 入参透传')"`

	// 定义API后端服务
	BackendType                types.BackendType                `xorm:"VARCHAR(100) not null comment('HTTP，RPC，MOCK')"`
	...
	ServiceHTTPMethod          types.HTTPMethod                 `xorm:"VARCHAR(50) comment('请求方法：GET，POST，PATCH，PUT，DELETE，HEAD，OPTIONS，ANY')"`
	ServiceTimeout             int32                            `xorm:"INT(11) default 0 comment('后端超时，单位ms')"`
	ServiceContentTypeCatagory types.ServiceContentTypeCatagory `xorm:"VARCHAR(255) comment('后端content-type类型：CLIENT 透传客户端ContentType头，CUSTOM 自定义，DEFAULT API网关默认')"`
	...
	// 返回结果定义
	ResultType       types.ResultType `xorm:"VARCHAR(100) not null comment('返回ContentType：JSON，TEXT，BINARY，XML，HTML，PASSTHROUGH（透传后端Content-Type）')"`
...
type ApiRequestParam struct {
	...
	ParameterLocation types.ParameterLocation `xorm:"TINYINT(2) not null comment('参数位置：1 Parameter Path，2 Head，3 Query，4 Body')"`
	ParameterType     types.ParameterType     `xorm:"TINYINT(2) not null comment('类型：1 String，2 Int，3 Long，4 Float，5 Double，6 Boolean 7 Array')"`
	IsRequired        types.ParamIsRequired   `xorm:"TINYINT(2) default 1 comment('是否必填：1 是，2 否')"`
	DefaultValue      string                  `xorm:"VARCHAR(255) comment('默认值')"`
	DemoValue         string                  `xorm:"VARCHAR(255) comment('示例')"`
	Description       string                  `xorm:"VARCHAR(255) comment('描述')"`
	DocShow           types.DocShow           `xorm:"TINYINT(2) default 1 comment('文档可见：1 不可见，2 可见')"`
...
type ApiRequestParamArray struct {
	ArrayItemsType    types.ParamArrayItemsType `json:"arrayItemsType"`    // 数组字段类型：String，Int，Long，Float，Double，Boolean，File
	MaxLength         int32                     `json:"maxLength"`         // 最大长度
	EnumValue         string                    `json:"enumValue"`         // 枚举，用逗号(,)隔开
	RegularExpression string                    `json:"regularExpression"` // 参数验证，正则表达式
}
...
type ApiServiceParamConstParam struct {
	...
	Location      types.ParameterLocation `json:"location"`      // 参数位置，2 Head，3 Query
	ParameterType types.ParameterType     `json:"parameterType"` // 参数类型
